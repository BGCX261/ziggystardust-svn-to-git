...
public class DataFilterApprise implements IDataRepository,IProviderUser{
    ...
    private HashSet<String> variablesSeen = new HashSet<String>();
    ...
    private void newVariableCheck(ValueObject val){
        ...
        if(variablesSeen.add(val.getVariable().toLowerCase()))
    ...
    private void newBadValue(ValueObject val){
        ...
        if(countFiltered%100==0){
            logger.info(countFiltered + " bad values removed. " + val.getSite());
        }
        
        //No longer keep list of values filtered
