...
public class DataParserGleonServiceXML extends GenericDataSource{
    ...
    //milliseconds representation of when privateStart begins
    private final int startAtFixedRateOf = 1000*60*10;
    ...
    public void removeSource (XmlSource x) {
        if (sources.contains(x)) {
            logger.info("Deleting source name: " + x.sourceName);
            sources.remove(x);
            raiseEvent(events.updated);
        }
    }
    ...
    public class XmlSource{
        ...
        private Date lastCollectionAttempt;
        private Date lastDataPointDownloaded; //so far, this is unused, this might change
        private Date nextCollectionAttempt;
        private int numValuesCollected;
        ...
        public Integer getNumValuesCollected() {
            return this.numValuesCollected;
        }

        public Date getNextCollectionAttempt() {
            return this.nextCollectionAttempt;
        }

        public Date getLastDataPointDownloaded() {
            return this.lastDateRetrieved;
        }

        public Date getLastCollectionAttempt() {
            return this.lastCollectionAttempt;
        }
        ...
        public void tryRetrieval(){
            Calendar cal = Calendar.getInstance();
            lastCollectionAttempt = cal.getTime();
            cal.add(Calendar.MINUTE, startAtFixedRateOf/1000/60);
            nextCollectionAttempt = cal.getTime();
            cal.add(Calendar.MINUTE, -(startAtFixedRateOf/1000/60));
